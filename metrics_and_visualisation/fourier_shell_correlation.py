...
def conical_fourier_shell_correlation_split_computation(im1, im2, axes, nb_radius, coeff_kernel_angles, sigma_radius, side=None, set_size=20, transposition=(1,2,0)):
    ...
    for i in range(nb_sets):
        ...
        t = time()
        conical_fsc_set, radiuses_frequencies = conical_fourrier_shell_correlation(im1, im2, set_axes, nb_radius, coeff_kernel_angles,
                                                                                sigma_radius, side=side, transposition=transposition)
        conical_fsc[:, start_idx:end_idx] = conical_fsc_set
    return conical_fsc, radiuses_frequencies


def conical_fourrier_shell_correlation(im1, im2, axes, nb_radius, coeff_kernel_angles, sigma_radius, side=None, transposition=(1,2,0)):
    ...
    # kernel_angles_threshold  = 1*(kernel_angles>=0.5*np.max(kernel_angles))
    ...
    kernel_angles_repeated = np.repeat(np.expand_dims(kernel_angles, axis=0), kernel_radius.shape[0], axis=0)
    kernel_radius_repeated = np.repeat(np.expand_dims(kernel_radius, axis=1), kernel_angles.shape[0], axis=1)
    global_kernel = kernel_angles_repeated * kernel_radius_repeated
    # print('ma global kernel', np.max(global_kernel))
    ...
    F1_repeated = np.repeat(np.repeat(F1_flatten, global_kernel.shape[0], axis=0), global_kernel.shape[1], axis=1)
    F2_repeated = np.repeat(np.repeat(F2_flatten, global_kernel.shape[0], axis=0), global_kernel.shape[1], axis=1)

    """computes the conical fourier shell correlation"""
    numerator = np.sum(global_kernel * np.real(F1_repeated * np.conj(F2_repeated)), axis=2)
    first_factor_denom = np.sum(global_kernel * np.real(F1_repeated * np.conj(F1_repeated)), axis=2)
    second_factor_denom = np.sum(global_kernel * np.real(F2_repeated * np.conj(F2_repeated)), axis=2)
    denominator = np.sqrt(first_factor_denom * second_factor_denom)
    conical_fsc = numerator / denominator
    return conical_fsc, radiuses


def plot_map(vals_to_plot, thetas, phis, vmin=None, vmax=None, cmap_type='cool', **color_bar_args):
    ...
    plt.imshow(np.nan_to_num(grid_z.T, nan=mean_non_nan), extent=(min_theta, max_theta, min_phi, max_phi),
               origin='lower', cmap=plt.cm.get_cmap(cmap_type), vmin=vmin, vmax=vmax)

    # create an axes on the right side of ax. The width of cax will be 5%
    # of ax and the padding between cax and ax will be fixed at 0.05 inch.
    #divider = make_axes_locatable(ax)
    #cax = divider.append_axes("right", size="5%", pad=0.05)
    cbar = plt.colorbar(fraction=0.025, pad=0.04, **color_bar_args)
    cbar.formatter.set_powerlimits((0, 0))
    """
    plt.imshow(np.nan_to_num(grid_z.T, nan=mean_non_nan), extent=(min_theta, max_theta, min_phi, max_phi),
               origin='lower', cmap=plt.cm.get_cmap(cmap_type), vmin=vmin, vmax=vmax)  # , origin='lower', extent=(0, 1, 0, 1))
    plt.colorbar(**color_bar_args)
    """
...
def plot_resolution_map(radiuses_frequencies, thetas, phis, conical_fsc, cutoff=CUTOFF_DEFAULT_VAL, vmin=0,vmax=0.5, cmap_type='coll', **color_bar_args):
    radiuses_cut_off = find_cutoffs_conical_fsc(radiuses_frequencies, conical_fsc, cutoff)
    plot_map(radiuses_cut_off, thetas, phis, vmin, vmax, cmap_type=cmap_type, **color_bar_args)
    return radiuses_cut_off
...
if __name__ == '__main__':
            ...
            for spartran in [False, True]:
                ...
                for fsc_view_gt in [True, False]:
                    ...
                    if sim_data:
                        ...
                        sz = 50
                        _, im1 = generate_image_with_ft(coeffs_1, frequencies_1, sz, 1) + np.random.normal(0, sig_noise, (sz, sz, sz))
                        _, im2 = generate_image_with_ft(coeffs_2, frequencies_2, sz, 1) + np.random.normal(0, sig_noise, (sz, sz, sz))
                        save('im_sin.tif', im1)
                    ...
                    conical_fsc, radiuses_frequencies = conical_fourier_shell_correlation_split_computation(im1, im2, axes, nb_radius, coeff_kernel_axes, sig_radus,
                    ...
                    if not real_data_reconstructions:
                        if not sim_data:
                            ...
                            save_path = f'{PTH_ILLUSTRATIONS}/conical_fsc_abs_dot_3/{part_name}'
                            if fsc_view_gt:
                        ...
                        save_name = 'assembly'
                        title = ''
                    save_name += f'coeff_{coeff_kernel_axes}'
                    if use_given_axes:
                        save_name += '_given_axis'
                    plot_graphs(radiuses_frequencies, conical_fsc.T, 'frequencies (1/pixel)', 'correlation coefficient', title, labels, save_path, 'cfsc_graphs.png')
                ...
                if len(conical_fsc_view_recons)==2:
                    ...
                    make_dir_and_write_array(np.array([[measure_of_isotropy_recons]]), f'{save_path}/measure_of_isotropy', f'{save_name}_recons.csv')
    ...
    """
    import matplotlib.pyplot as plt
    import manage_matplotlib.font_size
    gt_name1 = 'recepteurs_AMPA'
    gt_name2 = 'clathrine'
    path_gt = f"/home/eloy/Documents/stage_reconstruction_spfluo/ground_truths/{gt_name1}.tif"
    # path_view = '/home/eloy/Documents/stage_reconstruction_spfluo/views/recepteurs_AMPA/nb_views_1/sig_z_5/set_5/view_r_218.5_135.4_323.9_t_0.0_0.0_0.0_.tif'
    path_view_1 = f"/home/eloy/Documents/stage_reconstruction_spfluo/views/{gt_name1}/single_view/sig_z_5/view_1.tif"
    path_view_2 = f"/home/eloy/Documents/stage_reconstruction_spfluo/views/{gt_name1}/single_view/sig_z_5/view_3.tif"
    pth_1 = f'/home/eloy/Documents/stage_reconstruction_spfluo/results_hpc/test_gt/{gt_name1}/test_6/recons_registered.tif'
    pth_2 = f'/home/eloy/Documents/stage_reconstruction_spfluo/results_hpc/test_gt/{gt_name1}/test_7/recons_registered.tif'
    pth_3 = f'/home/eloy/Documents/stage_reconstruction_spfluo/results_hpc/test_gt/{gt_name2}/test_9/recons_registered.tif'
    gt = io.imread(path_gt)
    view1 = io.imread(path_view_1)
    view2 = io.imread(path_view_2)
    ...
    fsc1 = calc_fsc(im1, im1, 1)
    
    fsc2 = calc_fsc(im2, im3, 1)
    fsc_view_gt = calc_fsc(view, gt, 1)
    fsc_recons_gt = calc_fsc(im1, gt, 1)
    res_view_gt, x, y, _ = fsc2res(fsc_view_gt, return_plot=True)
    plt.plot(x, y, label='fsc between one view and the ground truth image')
    plt.xlabel('frequencies ')
    res_recons_gt, x, y, _ = fsc2res(fsc_recons_gt, return_plot=True)
    plt.plot(x, y, label='fsc between the reconstructed volume and the ground truth image')
    plt.legend()
    plt.show()
    print('fsc between one view and the ground truth image', res_view_gt)
    print('fsc between the reconstructed volume and the ground truth image', res_recons_gt)
    resn , x, y, resx = fsc2res(fsc1, return_plot=True)
    print('res 2 identical images', resn)
    resn_2 = fsc2res(fsc2)
    print('res 2 different images', resn_2)
    plt.plot(x, y)
    plt.show()
    """
